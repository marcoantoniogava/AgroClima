...
-------------------------------------------------- */
// Busca as informações da propriedade salvas durante a navegação anterior
// Valores padrão são usados caso não encontre nada
var propriedade = {
...
/* --------------------------------------------------
   Header
-------------------------------------------------- */
// Atualiza o título da página com o nome da propriedade
document.getElementById('headerNome').textContent = propriedade.nome;

// Se houver localidade, atualiza o subtítulo
if (propriedade.local) {
    document.getElementById('headerLocalTxt').textContent = propriedade.local;
}

// Configura o botão de voltar para retornar à lista de propriedades
...
-------------------------------------------------- */
// Controla qual aba está ativa na tela (24h ou 7 dias)
var periodoAtivo = '24h';

/* --------------------------------------------------
   Nomes dos dias da semana
-------------------------------------------------- */
// Arrays usados para formatar as datas
...
function carregarDados() {
    // Faz a chamada à API do Open-Meteo passando as coordenadas da propriedade
    $.ajax({
        ...
        data: {
            ...
            // Solicita os dados que afetam as operações agrícolas por hora e por dia
            ...
            forecast_days: 8 // Busca 8 dias para cobrir 7 dias inteiros
        },
        dataType: 'json',
        success: function (dados) {
            processarDados(dados); // Direciona a resposta da API para a função de tratamento
        },
        error: function () {
            // Em caso de erro, exibe uma mensagem no lugar do ícone de carregamento
...
function processarDados(dados) {
    ...
    // Começa a extrair os dados a partir da hora atual do sistema
    var idxAtual = horaAtual;

    // 1. Monta o array de dados para as próximas 24 horas
    var horas24 = [];
    for (var i = 0; i < 24; i++) {
        var idx = idxAtual + i;
        if (idx >= horario.time.length) break; // Evita estourar o limite do array retornado
        horas24.push({
            idx: idx,
            ...
            atual: i === 0 // Sinaliza se é a hora atual para destaque visual
        });
    }

    // 2. Monta o array de dados para os próximos 7 dias
    var dias7 = [];
    for (var d = 0; d < 7; d++) {
        ...
        dias7.push({
            ...
            // Como a API pode não retornar vento médio sempre, usa uma estimativa baseada no máximo caso falte
    ...
    // 3. Renderiza os três grandes blocos de operações (Pulverização, Plantio e Colheita)
    ...
    // Esconde a tela de carregamento e exibe o conteúdo gerado
    document.getElementById('estadoLoading').style.display = 'none';
    document.getElementById('conteudoJanelas').style.display = '';

    // Configura os botões que alternam entre a visão de 24h e 7 dias
...
// Controla a visibilidade das divs correspondentes a 24h e 7 dias para todas as operações
...
// Função centralizadora que cria o HTML de um bloco de operação específico
// Recebe o ID do bloco e as funções de avaliação específicas dele (ex: avaliarPulv)
function renderizarBloco(id, horas24, dias7, fnAvaliarHora, fnIndicadores) {
    var idCap = id.charAt(0).toUpperCase() + id.slice(1); // Ex: 'pulv' -> 'Pulv'

    // Define o status geral da operação avaliando as próximas 6 horas
    ...
    // Regra simples: Se tiver 3h ou mais ruins, status geral é desfavorável.
    // Se tiver 3h ou mais boas, favorável. Senão, fica em atenção.
    var statusGeral = contagem.desfavoravel >= 3 ? 'desfavoravel'
        : contagem.favoravel >= 3 ? 'favoravel'
            : 'atencao';

    // Atualiza a "badge" (etiqueta colorida) no topo do card
...
// Gera o HTML da barra horizontal que mostra hora a hora
function renderTimeline(horas24, fnAvaliar) {
    ...
    horas24.forEach(function (h, i) {
        var status = fnAvaliar(h); // Avalia a condição climática desta hora
        
        // Identifica onde começa e onde termina a janela de tempo boa (verde)
        if (status === 'favoravel') {
            if (primeiraFav === -1) primeiraFav = i;
            ultimaFav = i;
        }
        
        // Monta o bloquinho de cada hora
    ...
    // Se encontrou alguma hora boa, mostra o resumo. Senão, avisa que não tem janela.
    html += primeiraFav !== -1
...
// Gera o HTML para a visão de 7 dias
...
/* --------------------------------------------------
   Regras de avaliação (Critérios Agronômicos)
-------------------------------------------------- */
// Retorna se é bom ou ruim pulverizar baseado no clima da hora (h)
function avaliarPulv(h) {
    if (h.vento > 15 || h.umid < 40 || h.chuva > 0.2 || h.temp > 32) return 'desfavoravel'; // Regras excludentes
    if (h.vento >= 3 && h.vento <= 10 && h.umid >= 50 && h.chuva < 0.05 && h.temp < 30) return 'favoravel'; // Cenário ideal
    return 'atencao'; // Meio termo
}

// Monta os 4 indicadores do cabeçalho de pulverização
...
// Avaliação para Plantio (Foca mais em chuva extrema ou calor excessivo)
function avaliarPlant(h) {
    if (h.chuva > 5 || h.temp > 35) return 'desfavoravel';
    if (h.chuva >= 0.1 && h.chuva <= 2 && h.temp >= 15 && h.temp <= 30) return 'favoravel'; // Chuva leve ajuda a assentar a semente
    return 'atencao';
}
...
// Avaliação para Colheita (Odeia chuva e umidade alta)
...
// Função de apoio que transforma a lista de indicadores e avaliações em HTML
...
function iconeStatus(s) {
    if (s === 'favoravel') return '<svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>'; // Check
    if (s === 'atencao') return '<svg viewBox="0 0 24 24"><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>'; // Exclamação
    return '<svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>'; // X
}
