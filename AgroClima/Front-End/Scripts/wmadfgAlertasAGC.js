/* ========================================
   AGROCLIMA - Tela de Alertas (AC004)
   JS principal
   ======================================== */
   ...
   $(document).ready(function () {
    ...
    /* --------------------------------------------------
       Carrega propriedades e verifica alertas
    -------------------------------------------------- */
    function iniciar() {
    ...
    /* --------------------------------------------------
       Verifica condições climáticas — sequencial para
       evitar race condition no p_salvarAlertas
    -------------------------------------------------- */
    function verificarAlertas(propriedades) {
        ...
        function processarProxima() {
            if (indice >= propriedades.length) {
                // Todas processadas — salva ou exibe
            ...
            $.ajax({
                ...
                complete: function () {
                    // Chama a próxima só após esta terminar — elimina concorrência
                    processarProxima();
                }
            });
        }
    ...
    function analisarCondicoes(dados, prop) {
        ...
        if (chuva24h > 5) {
            alertas.push({ tipo: 'CHUVA',
                descricao: 'Chuva prevista nas proximas 24 horas (' + chuva24h.toFixed(1) + 'mm). Evite realizar pulverizacoes.',
        ...
        if (dados.hourly && dados.hourly.windspeed_10m) {
            ...
            if (ventoMax > 15) {
                alertas.push({ tipo: 'VENTO',
                    descricao: 'Vento forte detectado (' + Math.round(ventoMax) + ' km/h). Alto risco de deriva durante pulverizacao.',
                    data: hoje, hora: hora, idPropriedade: prop.id, nomePropriedade: prop.nome,
                    lat: prop.latitude, lon: prop.longitude });
            }
        }
        ...
        if (dados.hourly && dados.hourly.relativehumidity_2m) {
            ...
            if (umidMin < 40) {
                alertas.push({ tipo: 'UMIDADE',
                    descricao: 'Umidade baixa detectada (' + Math.round(umidMin) + '%). Risco de evaporacao durante aplicacao agricola.',
                    data: hoje, hora: hora, idPropriedade: prop.id, nomePropriedade: prop.nome,
                    lat: prop.latitude, lon: prop.longitude });
            }
        }
        ...
        $.ajax({
            ...
            success: function (vresult) {
                ...
                try {
                    ...
                    todosAlertas = [];
                    renderizarAlertas();
                    console.log('Erro ao parsear alertas:', e);
                }
            },
